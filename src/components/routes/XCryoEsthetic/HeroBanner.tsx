...
const query = graphql`
    query cryoEstethicHeroBannerImages {
        bgVideo: file(relativePath: { eq: "video/mask_bg.mp4" }) {
            publicURL
        }
        fullVideo: file(relativePath: { eq: "video/mask.mp4" }) {
            publicURL
        }
...
const HeroBanner = () => {
    ...
    const { openModal: openVideoModal } = useModal('maskVideoPlayer');
    ...
    return (
        <HeroBannerWithVideoBG url={bgVideo.publicURL}>
            ...
            <VideoPlayerModal name="maskVideoPlayer" src={fullVideo.publicURL} />
